...
- Do not force a category onto generic, infrastructure-agnostic hardware. A general-purpose server, workstation, or compute/storage box with no inherent media-workflow function of its own (it becomes whatever software is installed on it - could run a MAM, an encoder, or a playout system, and the source material gives no indication which) should leave main_category as an empty string. Do not guess based on what the hardware could theoretically be used for - only categorize when the source material itself describes an actual media-workflow function the product performs.
- "Support" specifically means broadcast/media signal monitoring and testing tools (waveform monitors, signal analyzers, QC/compliance tools). A built-in remote management/health-monitoring controller chip on a generic server (e.g. Lenovo XClarity, Dell iDRAC, HPE iLO) is a standard IT hardware-management feature present on nearly all enterprise servers regardless of purpose - it does not qualify as "Support" and does not make an otherwise infrastructure-agnostic server fit that category.
...
- Publish vs Monetize disambiguation: automated channel branding, promo insertion, and station identification tools are Publish, not Monetize - the deciding question is whether the inserted content is the broadcaster own branding/promotional material (Publish) versus paid third-party advertising (Monetize). A tool that inserts ads, tracks ad performance, or manages ad sales/pricing is Monetize. A tool that inserts bugs, lower-thirds, promos for that same channel upcoming shows, or station idents is Publish, even though the underlying insertion mechanism may be technically similar.
