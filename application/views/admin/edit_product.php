...
<style type="text/css">
   ...
   box-shadow: 0 0 0 2px white, 0 0 0 3px #0A66C2;
   }
   /*headings*/
   .fs-title {
   font-size: 15px;
...
<div class="content-wrapper">
   ...
   <section class="content-header">
      ...
      <ol class="breadcrumb">
         <li><a href="<?php echo base_url(); ?>"><i class="fa fa-dashboard"></i>  Product</a></li>
  ...
  <fieldset id="menu1" class="display_block" style="display: block;">
    ...
    <div class="row">
            ...
            <div class="col-sm-6">
              ...
              <div class="form-group">
                      <label for="title">( Software only <?php 

                        if($product_detail['latest_firmware_version']==0) { 
echo '<input  id="checkme" type="checkbox" value="1" name="latest_firmware_version" >';
                      }else{
echo '<input checked id="checkme" type="checkbox" value="0" name="latest_firmware_version">';
                      }?>
                       ) </label>
                      <!-- <input type="text"  class="form-control" autocomplete="off" onkeyup="getprocess_latest_firmware_version()" id="latest_firmware_version" name="latest_firmware_version" value="<?=$product_detail['latest_firmware_version']?>" >

                      <input type="hidden" name="latest_firmware_versionid" id="latest_firmware_versionid" /> 
                          <ul id="latest_firmware_versionSugguestion" ></ul> -->
                  </div>
...
<div class="form-group">
                    ...
                    <input type="file"  class="form-control" accept="application/pdf"  name="device_manual_brochure" value="<?=$product_detail['device_manual_brochure']?>" >
                  </div>
...
<div class="autocomplete" >
                          ...
                          <input data-toggle="#hidden1" class="form-control rui-input rui-location-box rui-auto-complete-input"   autocomplete="off" placeholder="" value="<?=$product_detail['device_brand']?>" id="device_brand" name="device_brand" >                        
</div>
              </div>
                  ...
                  <div class="form-group">
                    ...
                    <!-- <input type="date" class="form-control" name="date_released" > -->
                      <div class='right-inner-addon  date datepicker' data-date-format="yyyy-mm-dd">
                                    <input   type="text"  id="my_date_picker"  min="<?php echo date('Y-m-d');?>" value="<?=$product_detail['date_released']?>"  name="date_released" placeholder="" class="form-control">
  ...
  <div class="form-group">
                      ...
                      <input type="text"  class="form-control" autocomplete="off" onkeyup="getprocess_release_version()" id="release_version" name="release_version" value="<?=$product_detail['release_version']?>">
...
<button type="submit"  data-toggle="modal"  value="submit" class="btn submit_btn submitBtn">Submit</button>
 ...
 -->
    ...
    <a onclick="return (confirm('Are you sure?'))" href="<?php echo base_url();?>Admin/edit-product/<?php echo $product_detail['id'] ?>" class="submit action-button actionButtonSubmit" >Cancel</a>
  </fieldset>
</form>
...
$(document).ready(function() {
  ...
  $(".processsuggestionStand").select2({ tags: true, tokenSeparators: [','] });
   ...
   // e.preventDefault();
    ...
    $.ajax({
      method: "POST",
      url: site_url+"Admin/Product/edit_product_action?action=update_shop_product",
      data: formData,
      dataType: "JSON",
 ...
 $.ajax({
    ...
    success:function(data)
    {
       ...
       yearRange: '-115:+10',
               ...
               var min = new Date(),
     strMin = $.datepicker.formatDate("Y-m-d", min);
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    jQuery(".instand").typeahead({
      source: function (query, process) {
        return $.get('<?php echo base_url(); ?>Admin/Product/inputProcessSatndardSugguestion', { query: query }, function (data) {
          data = $.parseJSON(data);
          return process(data);
        });
      },
    ...
    jQuery(".inprocessConnection").typeahead({
      source: function (query, process) {
        return $.get('<?php echo base_url(); ?>Admin/Product/inputconnectionTypeSugguestion', { query: query }, function (data) {
          data = $.parseJSON(data);
          return process(data);
        });
      },
  ...
  var tagsO = $(".outputF").tagsManager();
    jQuery(".outputF ").typeahead({
      source: function (query, process) {
        return $.get('<?php echo base_url(); ?>Admin/Product/outputSugguestion', { query: query }, function (data) {
          data = $.parseJSON(data);
          return process(data);
        });
      },
    ...
    jQuery(".otstand").typeahead({
      source: function (query, process) {
        return $.get('<?php echo base_url(); ?>Admin/Product/outputProcessSatndardSugguestion', { query: query }, function (data) {
          data = $.parseJSON(data);
          return process(data);
        });
      },
    ...
    jQuery(".otprocessConnection").typeahead({
      source: function (query, process) {
        return $.get('<?php echo base_url(); ?>Admin/Product/outputconnectionTypeSugguestion', { query: query }, function (data) {
          data = $.parseJSON(data);
          return process(data);
        });
      },
    ...
    jQuery(".processsuggestion").typeahead({
      source: function (query, process) {
        return $.get('<?php echo base_url(); ?>Admin/Product/processsuggestion', { query: query }, function (data) {
          data = $.parseJSON(data);
          return process(data);
        });
      },
    ...
    jQuery(".processsuggestionStand").typeahead({
      source: function (query, process) {
        return $.get('<?php echo base_url(); ?>Admin/Product/processStandardsuggestion', { query: query }, function (data) {
          data = $.parseJSON(data);
          return process(data);
        });
      },
